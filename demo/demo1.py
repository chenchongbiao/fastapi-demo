...
@app1.get("/query/bool/conversion")  # bool类型转换：yes on 1 True true会转换成true, 其它为false
def type_conversion(param: bool = False):
    return param

@app1.get("/query/validations")  # 长度+正则表达式验证，比如长度8-16位，以a开头。其它校验方法看Query类的源码
...
@app1.put("/request_body/city/{name}")
def mix_city_info(
    name: str,
    city01: CityInfo,
...
@app1.put("/request_body/multiple/parameters")
...
@app1.get("/cookie")  # 效果只能用Postman测试
